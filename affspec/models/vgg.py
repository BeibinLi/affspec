...
class VGGBlock(nn.Module):
    ...
    def __init__(self, nIn, nOut, stride=1, down_method='esp'): #down_method --> ['avg' or 'esp']
        ...
        super().__init__()
        self.stride = stride
        self.conv1 = conv3x3(nIn, nOut)
        self.bn1 = nn.BatchNorm2d(nOut)
        self.relu1 = nn.PReLU(nOut)
        self.conv2 = conv3x3(nOut, nOut, stride)
...
class VGG(nn.Module):
    ...
    def __init__(self, classes=1000, s=1):
        ...
        for i in range(reps[1]):
            self.level3.append(VGGBlock(config[2], config[2], stride=1))

        self.level4_0 = DownSampler(config[2], config[3], reinf=self.input_reinforcement) #out = 14
        self.level4 = nn.ModuleList()
        for i in range(reps[2]):
            self.level4.append(VGGBlock(config[3], config[3], stride=1))

        self.level5_0 = DownSampler(config[3], config[4]) #7
        self.level5 = nn.ModuleList()
        for i in range(reps[3]):
            self.level5.append(VGGBlock(config[4], config[4], stride=1))
