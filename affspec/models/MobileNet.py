...
class MobileNetBlock(nn.Module):
    ...
    def __init__(self, nIn, nOut, stride=1, down_method='esp'): #down_method --> ['avg' or 'esp']
        ...
        super().__init__()
        self.stride = stride
        self.conv1 = conv3x3(nIn, nIn, groups=nIn)
        self.bn1 = nn.BatchNorm2d(nIn)
        self.relu1 = nn.PReLU(nIn)
        self.conv2 = conv1x1(nIn, nOut, stride)
        self.bn2 = nn.BatchNorm2d(nOut)
        self.relu2 = nn.PReLU(nOut)
...
class DownSampler(nn.Module):
    ...
    def __init__(self, nin, nout, reinf=True):
        ...
        super().__init__()
        nout_new = nout - nin
        self.eesp = MobileNetBlock(nin, nout_new, stride=2, down_method='avg')
...
class MobileNets(nn.Module):
    ...
    def __init__(self, classes=1000, s=1):
        ...
        for i in range(reps[1]):
            self.level3.append(MobileNetBlock(config[2], config[2], stride=1))

        self.level4_0 = DownSampler(config[2], config[3], reinf=self.input_reinforcement) #out = 14
        self.level4 = nn.ModuleList()
        for i in range(reps[2]):
            self.level4.append(MobileNetBlock(config[3], config[3], stride=1))

        self.level5_0 = DownSampler(config[3], config[4]) #7
        self.level5 = nn.ModuleList()
        for i in range(reps[3]):
            self.level5.append(MobileNetBlock(config[4], config[4], stride=1))
