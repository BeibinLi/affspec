...
class EESPNet(nn.Module):
    ...
    def __init__(self, classes=1000, s=1):
        ...
        self.beibin_intro = nn.Conv2d(in_channels=3, out_channels=3, kernel_size = 7)
        ...
        self.level5.append(CBR(config[4], config[5], 1, 1, groups=1))#K[4]
    ...
    def forward(self, input, p=0.2):
        ...
        '''
#        beibin = self.beibin_intro(input)
        ...
        out = self.classifier(output_1x1)
...
if __name__ == '__main__':
    ...
    model = EESPNet(classes=1000, s=0.5)
    model = model.cuda()
    out = model(input)
    print('Output size')
